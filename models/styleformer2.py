...
from models.EncDec import DecoderLayer,Decoder,Encoder
...
class StyleFormer(nn.Module):
    ...
    def __init__(self,seq_l,pred_l, d_model,c_out,d_layers,m_layers,factor, dropout,n_heads):
        ...
        self.mapnet = MappingNet(seq_l,d_model)
...
#         super(MappingNet,self).__init__()
#         self.fc_input = torch.nn.Linear(seq_l,d_model)
        
#         self.fc_modules = nn.ModuleList()
#         for i in range(layer_n):
#             self.fc_modules.append(nn.Linear(d_model, d_model))
#             self.fc_modules.append(nn.ReLU())
#             self.fc_modules.append(nn.BatchNorm1d(d_model))

#         self.projection = nn.Linear(d_model, d_model)


#     def forward(self, x):
#         out = self.fc_input(x.permute(0,2,1))
